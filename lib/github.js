...
module.exports.getFile =
  async function getFile (graphQL, owner, repo, branch, path) {
    console.log(`getFile: owner, repo, branch, path: `, owner, repo, branch, path);
    try {
      const resp = await graphQL({
        query: ` query($org: String!, $repo: String!, $path: String!) {
      ...
      let readmeText
      // console.log(`resp.repository: `, resp.repository);
      ...
      error.code = 'GET_FILE_FAILED'
      Error.captureStackTrace(error, module.exports.getFile)
      throw error
    }
  }
...
async function getRemainingCommits (graphQL, owner, repo, branch, cursor) {
  try {
    let resp = await graphQL({
    ...
    const commits = !pageInfo.hasNextPage ? nodes : nodes.concat(await getRemainingCommits(graphQL, owner, repo, branch, pageInfo.endCursor))
