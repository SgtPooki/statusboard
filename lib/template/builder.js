...
function buildCss (outDir, tmplDir, config) {
  console.log('start:buildCss')
  return Promise.all(CSS.map(async (file) => {
    console.log(`building css for ${path.join('css', `${file}.css`)}`)
    try {
      ...
      console.log(`done building css for ${file}.css`)
      return `${config.baseUrl}/${path.relative(outDir, outPath)}`
    } catch (e) {
      console.log(`error building css for ${file}.css`)
      console.log(e)
    }
  }))
}
